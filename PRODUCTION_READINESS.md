...
| Listen | Temporarily hidden from public UI | Existing Supabase rooms, membership, reactions, song queue, owner archive/delete, and optional realtime refresh remain preserved | Keep hidden during payment-provider review; retest before a future re-enable |
...
| Digital badges | Code complete, externally gated | Three fixed one-time Talent7 Badge, Champion Badge, and Founder Badge digital purchases; server-verified permanent highest badge; payment history | Apply migration, configure Razorpay and Google Play, create Play products, connect webhooks/RTDN, and pass sandbox/Play-track tests before accepting money |
