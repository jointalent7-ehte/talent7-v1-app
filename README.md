...
Run the SQL files in `supabase/` in the canonical order documented in [supabase/MIGRATION_ORDER.md](supabase/MIGRATION_ORDER.md). Existing projects must apply only migrations they have not already run. For a database already current through migration 76, apply migrations 77 through 87 to restore Listen and gaming, add locality-based voice rooms and host microphone notifications, enable consent-based staffed and queued challenge rooms, add private AI-assisted breakdance scorecards, activate Talent7 League rewards, and publish editable Talent7 Passports. Legacy feature schemas remain in the history to preserve existing data; closing their UI routes does not authorize dropping their tables.
